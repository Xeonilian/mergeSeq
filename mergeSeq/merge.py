...
def main(folder="", primers=['27F', '1492R'], mod="auto", check_len=0, thredhold=30, break_num=3, fastacut=[20, 750], verbose=True, out="none"):
    """
    功能：
    ...
    """
    # 1 输入阶段--------
    # 判断folder是不是一个文件夹，如果是，提取里面全部的文件名保存到file_names
    ...
    if folder=="":
        ...
        if os.path.isdir(folder):
            ...
            file_names = [os.path.join(folder, file_name) for file_name in file_names]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out', type=str, default='none', help='output data: none- no output, merge- only output merge results, all- output all data to one file')
    # 解析参数
